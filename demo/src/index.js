...
function* fetchHeader() {
  try {
    winston.info('received fetchHeader request for url: ' + this.query.url);
    let dest = URL.parse(this.query.url);
    // todo: using url before defining
    if (!dest.protocol || !dest.hostname) {
      return {
        error: true,
        message: `invalid URL: ${this.query.url}`,
        url: dest.href
      };
    }
    if (isNonRoutableIp(dest.hostname)) {
      return {
        error: true,
        message: `non-routable IP address: ${dest.hostname}`,
        url: dest.href
      };
    }
    ...
    if (headers.length < 1) {
      winston.info('no policy found');
      return {
        error: true,
        message: `no CSP headers found at ${url.href}`,
        url: url.href
      };
    } else {
      ...
      for (let header of headers) {
        try {
          ...
          return {
            error: true,
            message: `CSP parsing error: ${ex.cause.getMessageSync()}`,
      ...
      winston.info('policy found: ' + policyText);
      return {
...
function* requestInput() {
  ...
  this.response.set('content-security-policy', "default-src 'none';script-src 'self' code.jquery.com maxcdn.bootstrapcdn.com;img-src 'self';font-src 'self' https://fonts.gstatic.com bootswatch.com;connect-src 'self';style-src 'self' bootswatch.com https://fonts.googleapis.com;frame-ancestors 'none';report-uri /csp-report"); //eslint-disable-line
}
...
function* directHeader() {
  ...
  if (!{}.hasOwnProperty.call(this.query, 'headerValue[]')) {
    return {
      error: true,
      message: 'no headerValue[] request parameter given'
    };
  }
  try {
    ...
    winston.info('directHeader policy: ' + finalPolicyText);
    info = {
    ...
    if (ex.cause) {
      ...
      info = {
        error: true,
        message: `${ex.cause.getMessageSync()}`
      };
    } else { // other error
...
var options = {
  ...
  ciphers: 'EECDH+ECDSA+AESGCM EECDH+aRSA+AESGCM EECDH+ECDSA+SHA384 EECDH+ECDSA+SHA256 EECDH+aRSA+SHA384 EECDH+aRSA+SHA256 EECDH+aRSA+RC4 EECDH EDH+aRSA RC4 !aNULL !eNULL !LOW !3DES !MD5 !EXP !PSK !SRP !DSS !RC4', //eslint-disable-line max-len
  honorCipherOrder: true
};
var port = process.env.PORT || 9080;
var tlsPort = process.env.TLS_PORT || 9443;
http.createServer(app.callback()).listen(port);
console.log('server started at port ' + port);
https.createServer(options, app.callback()).listen(tlsPort);
console.log('TLS server started at port ' + tlsPort);
